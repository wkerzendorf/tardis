...
def calculate_absorption_luminosities(
    ...
    if packets_mode == "virtual":
        # Virtual packets data - now from spectrum_solver after postprocessing
        if sim.spectrum_solver.virtual_packet_state is None:
            raise ValueError(
        ...
        vps = sim.spectrum_solver.virtual_packet_state
        packets_df = pd.DataFrame(
            {
                "nus": vps.nus.value,
                "energies": vps.energies.value,
        ...
        spectrum_delta_frequency = (
            sim.spectrum_solver.spectrum_virtual_packets.delta_frequency
        )
        plot_wavelength = (
            sim.spectrum_solver.spectrum_virtual_packets.wavelength
        )
        plot_frequency = (
            sim.spectrum_solver.spectrum_virtual_packets._frequency[:-1]
        )
        plot_frequency_bins = (
            sim.spectrum_solver.spectrum_virtual_packets._frequency
        )
        ...
        packets_df = pd.DataFrame(
            {
                "nus": transport_state.packet_collection.output_nus[
                    emitted_mask
                ],
        ...
        spectrum_delta_frequency = (
            sim.spectrum_solver.spectrum_real_packets.delta_frequency
        )
        plot_wavelength = sim.spectrum_solver.spectrum_real_packets.wavelength
        plot_frequency = sim.spectrum_solver.spectrum_real_packets._frequency[
            :-1
        ]
        plot_frequency_bins = (
            sim.spectrum_solver.spectrum_real_packets._frequency
        )
    ...
    line_mask = (
        packets_df["last_interaction_type"] > InteractionType.NO_INTERACTION
